...
function createBuilderStemCard(st, cfg){
  ...
  card.innerHTML = headerHTML + eqFilterHTML + volumeHTML + waveformHTML + volumeDialHTML + dragButtonHTML + genCreateButtonHTML + `\n        <div class="status-line hidden mt-2 text-sm text-white/80">Ready to create</div>\n      `
...
function setupEventListeners() {
  ...
  */
  ...
  // Improved with proper blob lifecycle management and validation
  document.addEventListener('dragstart', e => {
    ...
    try {
      ...
      console.log(`Drag URL created: ${filename}`)

      // Set drag data in multiple formats for compatibility
      // DownloadURL format: type:filename:url
  ...
  document.addEventListener('dragend', e => {
    ...
    // Clean up blob URL after a longer delay to ensure drag operation completes
    // Increased from 1s to 30s to prevent premature cleanup
    setTimeout(() => {
      if (stemBlobUrls[st]) {
        URL.revokeObjectURL(stemBlobUrls[st])
        delete stemBlobUrls[st]
